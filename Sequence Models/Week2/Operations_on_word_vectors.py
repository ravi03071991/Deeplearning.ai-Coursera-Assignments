...
def neutralize(word, g, word_to_vec_map):
    ...
    e_biascomponent = (np.dot(e, g) / np.dot(g, g.T)) * g
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    mu_B = mu.dot(bias_axis) / (bias_axis.dot(bias_axis.T)) * bias_axis
    mu_orth = mu - mu_B
    
    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = e_w1.dot(bias_axis) / bias_axis.dot(bias_axis.T) * bias_axis
    e_w2B = e_w2.dot(bias_axis) / bias_axis.dot(bias_axis.T) * bias_axis
  
    # Step 5: Adjust the Bias part of e_w1B and e_w2B using the formulas (9) and (10) given above (≈2 lines)
    corrected_e_w1B = np.sqrt(np.linalg.norm(1 - mu_orth.dot(mu_orth.T)))*(e_w1B - mu_B) / np.linalg.norm(e_w1 - mu_orth - mu_B)
    corrected_e_w2B = np.sqrt(np.linalg.norm(1 - mu_orth.dot(mu_orth.T)))*(e_w2B - mu_B) / np.linalg.norm(e_w2 - mu_orth - mu_B)
